    ...
    <div class="max-w-4xl mx-auto px-8 pt-8 pb-16">
      <div class="bg-zinc-900 border border-zinc-800 rounded-sm p-5 md:p-8">
        ...
        <div v-if="submitted" class="text-center py-8">
          <p class="text-emerald-300 font-bold text-lg mb-2">Show submitted!</p>
        ...
        <form v-else @submit.prevent="handleSubmit" class="space-y-6">
          <div class="grid grid-cols-1 sm:grid-cols-[160px_1fr] items-start gap-y-4 gap-x-6">
            ...
            <label class="text-sm font-semibold text-zinc-300 pt-2">Time</label>
            <div>
              <input
                v-model="showTime"
                type="text"
            ...
            <label class="text-sm font-semibold text-zinc-300 pt-2">Organizer</label>
            <div>
              <input
                v-model="organizer"
                type="text"
            ...
            <label class="text-sm font-semibold text-zinc-300 pt-2">Ticket URL</label>
            <div>
              <input
                v-model="ticketUrl"
                type="url"
          ...
          <div class="border-t border-zinc-800 pt-6">
            <div class="grid grid-cols-2 gap-6">
              <div>
                <label class="text-sm font-semibold text-zinc-300 mb-2 block">Genres</label>
                <p class="text-xs text-zinc-400 mb-2">e.g. "emo", "metal", "hardcore"</p>
                <input
                  v-model="genreInput"
                ...
                <div class="flex flex-wrap gap-2">
                  <span
                    v-for="g in genres"
                    :key="g"
                    class="flex items-center gap-1 px-3 py-1 rounded-full border border-zinc-700 text-sm text-zinc-300"
                  >
                    {{ g }}
                    <button type="button" @click="removeGenre(g)" class="text-zinc-500 hover:text-white">×</button>
                  </span>
                </div>
              </div>

              <div>
                <label class="text-sm font-semibold text-zinc-300 mb-2 block">Tags</label>
                <p class="text-xs text-zinc-400 mb-2">e.g. "all ages", "21+", "touring"</p>
                <input
                  v-model="tagInput"
                ...
                <div class="flex flex-wrap gap-2">
                  <span
                    v-for="t in tags"
                    :key="t"
                    class="flex items-center gap-1 px-3 py-1 rounded-full border border-zinc-800 text-sm text-zinc-400"
                  >
                    {{ t }}
                    <button type="button" @click="removeTag(t)" class="text-zinc-500 hover:text-white">×</button>
                  </span>
                </div>
              </div>
            </div>
